...
void accessData (
    ...
    mem_addr_t setIndexBits = ((1 << 4) - 1) & (addr >> b); 

    // Cache hit
    /* ... */
    if(cache[setIndexBits].tag == thisTag && cache[setIndexBits].valid == true)
    {
        (*hit_count)++; 
        return; 
    }
    ...
    /* ... */
    if(cache[setIndexBits].valid == false)
    {
        cache[setIndexBits].valid = true; 
        cache[setIndexBits].tag = thisTag; 
        (*miss_count)++; 
    }
