...
void accessData (
    ...
    if ( cache->occupancy == E ) {
        ...
        /* ... */
        cache_line_t* second = cache->front->next_cache_line; 
        free(cache->front); 
        cache->front = second; 
        cache->occupancy -= 1; 
        (*eviction_count)++; 
    ...
    /* ... */
    struct cache_line* new_node = (struct cache_line*) malloc (sizeof(struct cache_line)); 
    new_node->tag = tag; 
    new_node->next_cache_line = NULL; 

    if(cache->occupancy == 0)
    {
        cache->front = new_node; 
        cache->back = new_node; 
        cache->occupancy += 1;
        return; 
    }
