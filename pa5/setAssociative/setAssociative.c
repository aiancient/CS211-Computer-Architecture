...
void accessData (
    ...
    /* ... */
    mem_addr_t thisTag = addr >> (s+b);

    // Find the correct cache set
    /* ... */
    mem_addr_t setIndexBits = (addr>>b) & 3; 

    // Cache hit
    /* ... */
    for(int i = 0; i<4; i++)
    {
        if(cache[setIndexBits][i].tag == thisTag && cache[setIndexBits][i].valid == true)
        {
            (*hit_count)++; 
            cache[setIndexBits][i].recent_use = program_counter; 
            return;
        }
    }
    ...
    for(int i = 0; i<E; i++)
    {
        if(cache[setIndexBits][i].recent_use < tempPC)
        {
            tempLRU = i; 
            tempPC = cache[setIndexBits][i].recent_use;  
        }
    }

    
    if(cache[setIndexBits][tempLRU].valid == false)
    {
        cache[setIndexBits][tempLRU].tag = thisTag;
        cache[setIndexBits][tempLRU].recent_use = program_counter;
        cache[setIndexBits][tempLRU].valid = true;
        return; 
    }

    // If cache set line already in use, then record an eviction
    /* ... */
    if(cache[setIndexBits][tempLRU].tag != thisTag && cache[setIndexBits][tempLRU].valid == true)
    {
        cache[setIndexBits][tempLRU].tag = thisTag;
        cache[setIndexBits][tempLRU].recent_use = program_counter;   
        (*eviction_count)++;  
