...
void sorted_insert (
    ...
    if ( sorted_list_of_jobs == NULL || time < sorted_list_of_jobs->time ) {
        ...
        // Keep traversing linked list until you find a time slop that is greater than the current and if present we insert the current unless at the end
...
int main(int argc, char* argv[])
{
    ...
    //made this a ptr so its a straight comparison 
    struct node* current_node = sorted_list_of_jobs;

    //go thru the linked list and print out the names and then to free the memory to get rid of memory leaks 
